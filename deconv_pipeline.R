...
if ("DWLS" %in% deconv_to_use) {
  ...
  deconv_path = file.path(output_dir, "DWLS", paste0("deconv_DWLS_", single_cell_GSE_ID, "_downsample-", opt$downsample_n_cells))

  if (!file.exists(paste0(sig_path, ".rds"))) {
...
if ("MuSiC" %in% deconv_to_use) {
  ...
  sig_path = file.path(signatures_directory, "MuSiC", paste0("signature_", single_cell_GSE_ID, "_downsample-", opt$downsample_n_cells))
  deconv_path = file.path(output_dir, "MuSiC", paste0("deconv_MuSiC_", single_cell_GSE_ID, "_downsample-", opt$downsample_n_cells))

  if (!file.exists(paste0(sig_path, ".rds"))) {
    message(paste0("Computing MuSiC signature for ", single_cell_GSE_ID ))
    signature_MuSiC = omnideconv::build_model(
      single_cell_object = counts_mat,
      cell_type_annotations = labels,
      method = "MuSiC",
      bulk_gene_expression = bulk_tpm_mat,
    ...
    save_rds_mkdir(signature_MuSiC, paste0(sig_path, ".rds"))
    write_csv_mkdir(signature_MuSiC, paste0(sig_path, ".csv"))
  }else {
    message(paste0("Loading preexisting MuSiC signature for ", single_cell_GSE_ID))
    signature_MuSiC = readRDS(paste0(sig_path, ".rds"))
  }
  message(paste0("Running MuSiC deconvolution using ", single_cell_GSE_ID))
  deconvolution_MuSiC = omnideconv::deconvolute(
    bulk_gene_expression = bulk_tpm_mat,
    model = signature_MuSiC,
    method = "MuSiC",
    batch_ids = batch_ids
  )
  ...
  rm_if_exists("signature_MuSiC", "deconvolution_MuSiC")
  gc()
...
if ("CDSeq" %in% deconv_to_use) {
  ...
  sig_path = file.path(signatures_directory, "CDSeq", paste0("signature_", single_cell_GSE_ID, "_downsample-", opt$downsample_n_cells))
  deconv_path = file.path(output_dir, "CDSeq", paste0("deconv_CDSeq_", single_cell_GSE_ID, "_downsample-", opt$downsample_n_cells))

  if (!file.exists(paste0(sig_path, ".rds"))) {
    message(paste0("Computing CDSeq signature for ", single_cell_GSE_ID ))
    signature_CDSeq = omnideconv::build_model(
      single_cell_object = counts_mat,
      cell_type_annotations = labels,
      method = "CDSeq",
    ...
    save_rds_mkdir(signature_CDSeq, paste0(sig_path, ".rds"))
    write_csv_mkdir(signature_CDSeq, paste0(sig_path, ".csv"))
  }else {
    message(paste0("Loading preexisting CDSeq signature for ", single_cell_GSE_ID))
    signature_CDSeq = readRDS(paste0(sig_path, ".rds"))
  }
  message(paste0("Running CDSeq deconvolution using ", single_cell_GSE_ID))
  deconvolution_CDSeq = omnideconv::deconvolute(
    bulk_gene_expression = bulk_counts_mat,
    model = signature_CDSeq,
    method = "CDSeq",
    batch_ids = batch_ids
  )
